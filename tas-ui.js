(function createTasUI() {
    // 1. Configuration for HUD Panel Styling
    ...
    // 2. DOM Structure for the HUD Overlay
    ...
    hud.innerHTML = `
        ...
        <div style="border-top: 1px solid #333; padding-top: 6px;">
            ...
            <div class="direction-matrix">
                <div></div>
                <div class="matrix-btn" id="m-w">W</div>
                <div></div>
                <div class="matrix-btn" id="m-a">A</div>
                <div class="matrix-btn" id="m-s">S</div>
                <div class="matrix-btn" id="m-d">D</div>
            </div>
        </div>
        
        <div id="tas-log">-- System Active: Click Pause to step frame --</div>
    `;
    ...
    // Toggle Matrix Button Inputs on Clicki
    ['w', 'a', 's', 'd'].forEach(key => {
    ...
    // 4. CORE INTERCEPTION LOOP (Web Worker Proxy)
    // We overwrite Web Worker postMessage prototype to catch and modify data before it hits simulation_worker
    const originalWorkerPostMessage = Worker.prototype.postMessage;
    
    Worker.prototype.postMessage = function(message, transfer) {
        // messageType 6 = ControlCar packet sent from game engine to physics worker
        if (message && message.messageType === 6) {
            if (window.tasIsPaused) {
                // If a frame step hasn't been requested, block the message payload to freeze the loop
                if (!window.tasNextFrameRequested) {
                    return; 
                }
                
                // Override hardware engine array state with our custom TAS matrix variables
                ...
                // Reset step state values and advance counter indicators
                window.tasNextFrameRequested = false;
        ...
        // Fire original postMessage tracking if loop execution is allowed
